...
from normalize_input.match_email import *###################2
import traceback
...
def remove_punctuation(word: Text) -> Text:
    if '@' in word and '.' not in word:#############################2
        word=word.replace('@', ' a còng ')###########################2
    # number: 1,2 1.2 45,34 213.123.12321 12312,123.123 123.545,12312
    # number_pattern = r"[\(]{0,1}[0-9]{1,}[aA-zZ]{0,}([\.,\\/-][0-9]{1,}){1,}[\)]{0,1}"
    number_pattern = r"[\(]{0,1}[0-9]{1,}[aA-zZ]{0,}([\.,\\/-][0-9]{1,}){1,}[\)]{0,1}"
    if re.search(number_pattern, word) or '@' in word or 'http' in word:############################
        return word
...
def split_num_and_word(word: Text, list_words, idx, org_word) -> Text:
    ...
    groups = re.search(r'^([0-9]{0,})([a-zA-Z\\/]{1,})([0-9]{0,})([a-zA-Z\\/]{0,})', word)#đồng None##############2
    # print(word, groups)
    if groups is not None:
...
def split_num_and_word_email(word: Text, list_words, idx, org_word) -> Text:####################2
    # lmnguyen.12th212218m
    groups = re.search(r'^([0-9]{0,})([a-zA-Z\\/]{1,})([0-9]{0,})([a-zA-Z\\/]{0,})', word)#đồng None##############2
    # print(word, groups)
    if groups is not None:
        first_num = n2w(groups.group(1)) if groups.group(1) != '' else ''
        ...
        center_text = groups.group(2)
        last_num = n2w(groups.group(3)) if groups.group(3) != '' else ''
        last_text = groups.group(4) if groups.group(4) != '' else ''
        
        if first_num == '' and last_num == '':
            tmp_text = convert_upper_word_to_text(word)
        else:
            tmp_text = first_num + ' ' + convert_upper_word_to_text(center_text) + ' ' + last_num + ' ' + convert_upper_word_to_text(last_text)
    else:
        # Only number
        tmp_text = reformat_num_string(word, org_word, list_words, idx)
...
async def normalize_input_text_to_speech(input: Text, add_last_break: bool) -> Text:
    ...
    final_words = []
    print("process_sent", process_sent)
    ...
    for idx, org_word in enumerate(list_words):
        try:
            ...
            if word in common_conjunction.keys():
                ...
                tmp_text=''
                if prefix=='http':
                    tmp_text=" = hát-tê-tê-pê = hai-chấm gạch-chéo gạch-chéo "
                if prefix=='https':
                    tmp_text=" = hát-tê-tê-pê-ét = hai-chấm gạch-chéo gạch-chéo "
                link=link.replace("/", " gạch-chéo ").replace("."," chấm ").replace(":"," hai-chấm ")
                print("link", link)
                ...
                print(tmp_text)
                final_words.append(tmp_text.replace(" chấm ", " = chấm ") )
                ...
                list_sub_name=username.replace('.', ' chấm ').split(" ")#######.split(".")##################2
                tmp_text=''
                list_sub_name_match=[]
                list_flag_name=[]
                for sub_name in list_sub_name:
                    if sub_name == 'chấm':
                        list_sub_name_match.append(sub_name)
                        list_flag_name.append(1)
                    else:
                        name_match=match_username(sub_name)
                        list_sub_name_match.extend(name_match[0])
                        list_flag_name.extend(name_match[1])
                    
                for (sub_word, flag) in zip(list_sub_name_match,list_flag_name):
                        if(len(sub_word)>0):
                            print(list_sub_name_match)#####['', 'Hiệp', 'th212218m']
                            print(list_flag_name )############################ th hai-trăm-mười-hai-nghìn-hai-trăm-mười-tám m
                            if(flag==1):
                                print(sub_word)
                                tmp_text +=sub_word+ " "
                            else:    
                                tmp_text += split_num_and_word_email(sub_word, list_words, idx, org_word=sub_word) + " "

                # tmp_text = convert_upper_word_to_text(username, add_silent=True) + ' = ' + 'a còng ' + domain_text#########################2
                tmp_text = tmp_text + ' = ' + 'a còng ' + domain_text#########################2
                final_words.append(tmp_text)
