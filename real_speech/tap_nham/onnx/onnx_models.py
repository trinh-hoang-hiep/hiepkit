...
import attentions_onnx as attentions_onnx

from torch.nn import Conv1d, ConvTranspose1d, Conv2d
from torch.nn.utils import weight_norm, remove_weight_norm, spectral_norm
from commons_onnx import init_weights, get_padding
...
class TextEncoder(nn.Module):
  ...
  def forward(self, x, x_lengths):
    ...
    x_mask = torch.unsqueeze(commons_onnx.sequence_mask(x_lengths, x.size(2)), 1).to(x.dtype)
...
class SynthesizerTrn(nn.Module):
  ...
  def forward(self, x, x_lengths, sid=None, path=None):
    ...
    # random_tensor = torch.randn_like(m_p)
    # torch.save(random_tensor,"random_tensor.pt")
