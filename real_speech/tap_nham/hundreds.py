from normalize_input.vietnam_number_custom.number2word.data import units, units_don_vi
from normalize_input.vietnam_number_custom.number2word.units import n2w_units
import random
import copy
...
def n2w_hundreds(numbers: str):
    ...
    for e in range(0, len(reversed_hundreds)):
        ...
        if e == 0:
            total_number.append(units_don_vi[reversed_hundreds[e]])
        elif e == 1:
    ...
    #       8. 'hai trăm ba mươi lăm' sẽ bỏ ngẫu nhiên chữ 'mươi'
    print(total_number)
    for idx, value in enumerate(total_number):
        ...
        if idx == 0 and value not in ['không', 'một'] and total_number[1] != 'một-mươi-' and total_number[1] != 'không-mươi-':
            total_number[1] = total_number[1].replace('mươi-', random.choice(['mươi-', '']))
        ...
        if value == 'không-mươi-':
            if idx == len(total_number) - 1:
                # 02 => Không hai
                total_number[idx] = 'không '
            else:
                total_number[idx] = random.choice(['lẻ-', 'linh-'])

        if value == 'một-mươi-':
            total_number[idx] = 'mười-'

    tmp_text = ''.join(total_number[::-1]).strip()
    tmp_text = tmp_text.replace('mười-mốt', 'mười-một').replace('mười-tư', 'mười-bốn')
    if(tmp_text[-1]=="-"):
        tmp_text=tmp_text[0:-1]
    return tmp_text
